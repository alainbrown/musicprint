...
def audio_pipeline(sample_rate, window_samples, file_root=None, file_list=None, shard_id=0, num_shards=1, augment=False):
    ...
    if augment:
        ...
        # Ensure window_samples is also int64 to match anchor type
        audio = fn.slice(audio, anchor, fn.cast(window_samples, dtype=types.INT64), axes=[0], out_of_bounds_policy="pad")
        
    else:
        # Deterministic Start Crop
        audio = fn.slice(audio, 0, fn.cast(window_samples, dtype=types.INT64), axes=[0], out_of_bounds_policy="pad")
