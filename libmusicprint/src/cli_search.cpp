...
int main(int argc, char** argv) {
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " <query.bin> <index.bin> <meta.bin> <vocab.bin>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        // 4. Search
        auto results = searcher.search(query_hash, 1);
        if (results.empty()) {
